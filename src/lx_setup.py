...
if __name__ == '__main__':
    ...
    if args_num == 0:
        try:
            ...
            print('Error! Could not get servo ID information')
    elif args[0] == 'info':
    ...
    elif args[0] == 'sin_test' and args_num == 5:
        t = 0
        ctrl.move(int(args[1]), int(args[2]), 500)
        time.sleep(.4)
        ctrl.move(int(args[3]), int(args[4]), 500)
        time.sleep(.4)
        pos1 = []
        pos2 = []
        ts   = []
        while t < 10:
            ctrl.move(int(args[1]), int(args[2]) + sin(t)*140, 500)
            time.sleep(.4)
            ctrl.move(int(args[3]), int(args[4]) + cos(t)*140, 500)
            time.sleep(.4)
            pos1.append(ctrl.get_position(int(args[1])))
            pos2.append(ctrl.get_position(int(args[3])))
            ts.append(t)
            t += 0.1
        ctrl.move(int(args[1]), int(args[2]), 500)
        time.sleep(.4)
